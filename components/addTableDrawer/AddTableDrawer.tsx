...
const AddTableDrawer = ({ addTable, setAddDrawer }) => {
    ...
    const initCanvas = () => {
        const el = document.getElementById("tableConfigCanvasWrapper")?.getBoundingClientRect()
    ...
    const createTable = (tableWidth: number, tableHeight: number, seatTop: number, seatRight: number, seatBottom: number, seatLeft: number) => {
    ...
    useEffect(() => {
        ...
        if (canvas) {
            const table = createTable(150, 60, topSeat.at(0)!, rightSeat.at(0)!, bottomSeat.at(0)!, leftSeat.at(0)!)
            canvas.add(table)
            canvas.renderAll()
        }
        ...
        <div className="h-full">
            ...
            <div className="pb-3 mb-3 overflow-auto h-max">
                ...
                <div className='flex items-center gap-3 mt-1 mb-3 overflow-auto no-scroll-bar' >
                    <div className='flex-shrink-0 h-32 border shadow-sm w-52 rounded-xl' >
                    ...
                    <ul >
                        <li className='flex items-center mb-3'>
                            ...
                            <div className="w-full">
                                <Slider onValueChange={(val) => setTopSeat(val)} color="primary" value={topSeat} max={10} step={1} />
                            </div>
                        </li>
                        <li className='flex items-center mb-3'>
                            <p className="w-28">Right</p>
                            <div className="w-full">
                                <Slider onValueChange={(val) => setRightSeat(val)} value={rightSeat} color="primary" defaultValue={[1]} max={5} step={1} />
                            </div>
                        </li>
                        <li className='flex items-center mb-3'>
                            <p className="w-28">Bottom</p>
                            <div className="w-full">
                                <Slider onValueChange={(val) => setBottomSeat(val)} value={bottomSeat} color="primary" defaultValue={[3]} max={10} step={1} />
                            </div>
                        </li>
                        <li className='flex items-center mb-3'>
                            <p className="w-28">Left</p>
                            <div className="w-full">
                                <Slider onValueChange={(val) => setLeftSeat(val)} value={leftSeat} color="primary" defaultValue={[1]} max={5} step={1} />
                            </div>
                ...
                <Button onClick={() => handleAddTable()} className="float-right">Create table</Button>
            </div>
        </div>
    )
}
